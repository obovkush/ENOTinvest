...
module.exports = function (opt) {
  ...
  this.metadataCreds = grpc.credentials.createFromMetadataGenerator(function (
    args,
    callback,
  ) {
  ...
  this.loadPackageDefinition = function () {
    for (let pn of this.protoNames) {
      this.protoDescriptor[pn] = grpc.loadPackageDefinition(
        this.packageDefinition[pn],
      );
    }
  };
  ...
  this.loadContracts = function () {
    for (pn of this.protoNames) {
      this.contracts[pn] =
        this.protoDescriptor[pn].tinkoff.public.invest.api.contract.v1;
      ...
      for (let serviceName of Object.keys(this.contracts[pn])) {
        if (serviceName.indexOf('Service') > 0) {
          this[serviceName] = new this.contracts[pn][serviceName](
            this.url,
            this.ssl_creds,
          );
          var obj = this;
          ...
          for (let service of Object.keys(
            this.contracts[pn][serviceName].service,
          )) {
                ...
                obj[serviceName][service](opt, function (err, response) {
                  if (err !== null) {
                    if (
                      ...
                      err.metadata.internalRepr != undefined
                    ) {
                      ...
                      reject(
                        err +
                          ...
                          '"' +
                          ' \n x-ratelimit-limit: "' +
                          internal.get('x-ratelimit-limit') +
                          '"' +
                          ' \n x-ratelimit-remaining: "' +
                          internal.get('x-ratelimit-remaining') +
                          '"' +
                          ' \n x-ratelimit-reset: "' +
                          internal.get('x-ratelimit-reset') +
                          '"',
                      );
                    } else {
                      reject(err);
            ...
            this.types[
              obj.contracts[pn][serviceName]['service'][service]['path']
            ] = {
              requestType: this.getTypes(
                obj.contracts[pn][serviceName]['service'][service][
                  'requestType'
                ]['type']['field'],
              ),
              responseType: this.getTypes(
                obj.contracts[pn][serviceName]['service'][service][
                  'responseType'
                ]['type']['field'],
              ),
            ...
            for (let f of Object.keys(
              ...
              if (
                obj.contracts[pn][typeName] !== undefined &&
                obj.contracts[pn][typeName]['type']['field'] !== undefined
              ) {
                let fields = this.getTypes(
                  obj.contracts[pn][typeName]['type']['field'],
                );
                for (let f of Object.keys(fields)) {
                  this.types[
                    obj.contracts[pn][serviceName]['service'][service]['path']
                  ]['responseType'][f] = fields[f];
                }
              }
            }
            ...
            this[serviceNameShort][service] = function (opt) {
              return new Promise(function (resolve, reject) {
                if (opt.rules != undefined) {
                  obj.types[obj[serviceName][service]['path']]['requestType'] =
                    opt.rules;
                  delete opt.rules;
                }
                ...
                obj[serviceName][service](opt, function (err, response) {
                  ...
                  if (response !== undefined) {
                    response = obj.decodeResponse(
                      response,
                      obj.types[this.methodDefinition.path]['responseType'],
                    );
                    resolve(response);
                  } else {
                    resolve(0);
  ...
  this.decodeResponse = function (resp, rules) {
    ...
    for (let fieldName of Object.keys(resp)) {
      ...
      if (
        rules[fieldName] == 'google.protobuf.Timestamp' ||
        fieldName == 'date_time'
      ) {
